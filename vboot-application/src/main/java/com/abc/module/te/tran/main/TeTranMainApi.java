...
@RestController
@RequestMapping("te/tran/main")
public class TeTranMainApi {

    //分页特性分类
    @GetMapping
    public RestResult get(String name) {
        Sqler sqler = new Sqler("te_tran_main");
    ...
    public RestResult getOne(@PathVariable String id) {
        TeTranMain cate = service.findOne(id);
        return RestResult.ok(cate);
    }

    //新增分类
    @PostMapping
    public RestResult post(@RequestBody TeTranMain cate) {
        return RestResult.ok(service.insert(cate));
    }

    //修改分类
    @PutMapping
    public RestResult put(@RequestBody TeTranMain cate) {
    ...
    @Autowired
    private TeTranMainService service;
