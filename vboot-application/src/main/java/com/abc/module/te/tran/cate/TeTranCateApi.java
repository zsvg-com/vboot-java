...
@RestController
@RequestMapping("te/tran/cate")
public class TeTranCateApi {

    //分页特性分类
    @GetMapping
    public RestResult get(String name) {
        Sqler sqler = new Sqler("te_tran_cate");
    ...
    public RestResult getOne(@PathVariable String id) {
        TeTranCate cate = service.findOne(id);
        return RestResult.ok(cate);
    }

    //新增分类
    @PostMapping
    public RestResult post(@RequestBody TeTranCate cate) {
        return RestResult.ok(service.insert(cate));
    }

    //修改分类
    @PutMapping
    public RestResult put(@RequestBody TeTranCate cate) {
    ...
    @Autowired
    private TeTranCateService service;
