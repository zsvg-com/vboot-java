...
public class SysAuthPermService {
...
//        javaList.addAll(XmapUtil.getRootList());
        List<String> ordList = jdbcDao.findStringList("select id from sys_auth_perm");

        //收集最大权限位及最大权限位的最大权限码
        String sql = "select max(u.ipos) as ipos,max(u.lcode) as lcode from sys_auth_perm u where u.ipos = (select max(uu.ipos) from sys_auth_perm uu)";
        ...
        //更新数据库
        String initSql = "UPDATE sys_auth_perm SET avtag=0";
        String updateSql = "UPDATE sys_auth_perm SET name=?,pid=?,avtag=1 WHERE id=?";
        String insertSql = "INSERT INTO sys_auth_perm(id,pid,ipos,lcode,name,cotag,avtag) VALUES(?,?,?,?,?,?,1)";
        ...
        String g1permSql = "SELECT id,ipos,lcode,tcode from sys_auth_perm where avtag= 1 and cotag = 0 and lcode<>0 and id like '~G%' and id not like '%}'";
        this.g1perms = getPerms(g1permSql);

        String g2permSql = "SELECT id,ipos,lcode,tcode from sys_auth_perm where avtag = 1 and cotag = 0 and lcode<>0 and id like '~G%' and id like '%}'";
        this.g2perms = getPerms(g2permSql);

        String spermSql = "SELECT id,ipos,lcode,tcode from sys_auth_perm where avtag = 1 and cotag = 0 and lcode<>0 and id like '~PO%'";
        this.sperms = getPerms(spermSql);

        String upermSql = "SELECT id,ipos,lcode,tcode from sys_auth_perm where avtag = 1 and cotag = 0 and lcode<>0 and id like '~PU%'";
        this.uperms = getPerms(upermSql);

        String dpermSql = "SELECT id,ipos,lcode,tcode from sys_auth_perm where avtag = 1 and cotag = 0 and lcode<>0 and id like '~D%'";
        this.dperms = getPerms(dpermSql);

        String ppermSql = "SELECT id,ipos,lcode,tcode from sys_auth_perm where avtag = 1 and cotag = 0 and lcode<>0 and id like '~PA%'";
    ...
    public void initAdmin(){
        //admin用户
        String saSql="select u.usnam as id from sys_auth_label_org o inner join sys_org_user u on u.id=o.org inner join sys_auth_label l on l.id=o.id where l.code='admin'";
