package vboot.config.security.authn;

import cn.hutool.core.util.StrUtil;
import com.alibaba.fastjson.JSON;
import vboot.common.mvc.dao.JdbcDao;
import vboot.config.security.pojo.Duser;
import vboot.config.security.pojo.Zmenu;
import vboot.config.security.pojo.Zmeta;
...
//    }
    ...
    private List<Zmenu> findMenuList(String conds) {
        String sql = "select distinct m.name,m.code,m.path,m.icon,m.comp,m.ornum,m.id,m.pid,m.perm from sys_perm_menu m inner join sys_perm_role_menu rm on rm.mid=m.id inner join sys_perm_role_org ru on ru.rid=rm.rid where m.type in ('D','M') and m.avtag=1 and ru.oid in (" + conds + ") order by m.ornum";
        if(conds.contains("'sa'")||conds.contains("'vben'")){
            sql = "select m.name,m.code,m.path,m.icon,m.comp,m.ornum,m.id,m.pid,m.perm from sys_perm_menu m where m.type in ('D','M') and m.avtag=1 order by m.ornum";
        }
    ...
    private List<String> findBtnList(String conds) {
        String sql = "select distinct m.perm id from sys_perm_menu m inner join sys_perm_role_menu rm on rm.mid=m.id inner join sys_perm_role_org ru on ru.rid=rm.rid where m.type = 'B' and m.avtag=1 and ru.oid in (" + conds + ") order by m.ornum";
        if(conds.contains("'sa'")||conds.contains("'vben'")){
